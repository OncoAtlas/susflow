...
def read_national(
    year: int, destination: Path | None = None, force: bool = False, engine: str = "pandas"
) -> Any:
    """Download (if needed) and return the national aggregate as a DataFrame."""
    return _read(download_national(year, destination=destination, force=force), engine=engine)
...
def read_exception(
    ...
    return _read(download_exception(year, destination=destination, force=force), engine=engine)
